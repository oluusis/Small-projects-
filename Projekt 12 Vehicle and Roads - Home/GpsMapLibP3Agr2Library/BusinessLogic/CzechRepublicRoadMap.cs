...
namespace GpsMapLibP3Agr2Library.BusinessLogic
{
    ...
    public class CzechRepublicRoadMap : IRoadMap
    {



        // Paths to files. Change them if something goes wrong.
        public const string ResourcesDir = @"..\..\..\..\GpsMapLibP3Agr2Library\Resources\";
        public const string TownsAndCitiesCsv = ResourcesDir + @"townsAndCities.csv";
        public const string RoadsAllKindsCsv = ResourcesDir + @"roadsAllKinds.csv";
        ...
        public Gps MoveOnRoad(Gps currentLocation, string roadNo, bool fromAToB, decimal velocity, decimal timePeriod)
        {
            ...
            Town townToCheck;
            Road roadToCheck;
            WhereAmI(currentLocation, out townToCheck, out roadToCheck);
            bool isOk = false;
            // Are we on the road edge, in A?
            if ((townToCheck != null) && (townToCheck.Code == road.LocationA.Code))
            {
                isOk = true;
            }
            // Are we on the road edge, in B?
            else if ((townToCheck != null) && (townToCheck.Code == road.LocationB.Code))
            {
                isOk = true;
            }
            // Are we on the right road?
            else if ((roadToCheck != null) && (roadToCheck.RoadNo == road.RoadNo))
            {
                isOk = true;
            }
            ...
            if (!isOk)
            {
                throw new InvalidOperationException($"\'{currentLocation}\' not on the road between {road.LocationA.Name} and {road.LocationB.Name}.");
            }

            // (2) We're moving with an allowed speed.
            if ( ! ((road.MinVelocity <= velocity) && (velocity <= road.MaxVelocity)) )
            {
                throw new InvalidOperationException($"Minimum or maximum velocity violated! Your speed is: {velocity} km/h. But this road requires from {road.MinVelocity} to {road.MaxVelocity} km/h");
            }
        ...
        private void LoadTowns()
        {
            ...
            this.towns = fileService.LoadTownsAndCities(CzechRepublicRoadMap.TownsAndCitiesCsv);
        }
        ...
        {
            FileService fileService = new FileService();
            this.roads = fileService.LoadRoads(CzechRepublicRoadMap.RoadsAllKindsCsv, this.towns);
        }
